...
namespace SpeakEase.Infrastructure.Middleware
{
    ...
    public class ExceptionMiddleware(ILoggerFactory loggerFactory, IOptions<JsonOptions> options) : IMiddleware
    {
        ...
        public async Task InvokeAsync(HttpContext context, RequestDelegate next)
        {
            ...
            {
                await HandleExceptionAsync(context, ex, 499, options);
            }
            catch(RefeshTokenValidateException ex)
            {
                await HandleExceptionAsync(context, ex, 999, options);
            }
            catch (Exception ex)
            {
                await HandleExceptionAsync(context, ex, 500, options);
            }
            
        }

        private async Task HandleExceptionAsync(HttpContext context, Exception ex, int statusCode, IOptions<JsonOptions> options)
        {
